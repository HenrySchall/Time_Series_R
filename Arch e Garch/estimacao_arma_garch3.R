...
# Dados da ação VALE3.SA desde 01/01/2008
price_day <- quantmod::getSymbols("VALE3.SA", src = "yahoo", from = '2008-01-01')
log_day_return <- na.omit(PerformanceAnalytics::Return.calculate(VALE3.SA$VALE3.SA.Adjusted, method = "log"))
log_day_return_squad <- log_day_return^2

# Gráfico dos preços e retonos (observer a presença de heterocedasticidade condicional)
par(mfrow=c(2,1)) 
plot.xts(VALE3.SA$VALE3.SA.Adjusted, main = "Preços da VALE3", xlab = "tempo", ylab = "preços")
plot.xts(log_day_return, main = "Retornos da VALE3", xlab = "tempo", ylab = "retorno")

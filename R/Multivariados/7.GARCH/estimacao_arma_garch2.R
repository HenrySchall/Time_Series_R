...
# Dados da ação RENT3.SA desde 01/01/2015
price_day <- quantmod::getSymbols("RENT3.SA", src = "yahoo", from = '2015-01-01')
log_day_return <- na.omit(PerformanceAnalytics::Return.calculate(RENT3.SA$RENT3.SA.Adjusted, method = "log"))
log_day_return_squad <- log_day_return^2

# Gráfico dos preços e retonos (observer a presença de heterocedasticidade condicional)
par(mfrow=c(2,1)) 
plot.xts(RENT3.SA$RENT3.SA.Adjusted, main = "Preços da RENT3", xlab = "tempo", ylab = "preços")
plot.xts(log_day_return, main = "Retornos da RENT3", xlab = "tempo", ylab = "retorno")
...
# Como resultado temos que o modelo escolhido pelo BIC é o ARMA(0,1)
media_condicional <- arima(log_day_return, order = c(0,0,1), method = "ML")
...
media_variancia_condicional <- fGarch::garchFit(~arma(0,1)+garch(1,1), data = log_day_return, trace = FALSE, 
...
res = residuals(media_variancia_condicional, standardize = TRUE)
acf_residuals_armagarch <- acf(res^2, na.action = na.pass, plot = FALSE, lag.max = 20)
plot(acf_residuals_armagarch, main = "", ylab = "", xlab = "Defasagem")
title("FAC dos resíduos do arma(0,1)-GARCH(1,1)", adj = 0.5, line = 1)
Box.test(res^2,type="Ljung",lag=1)

# Previsão do retorno esperado e variância esperada
fGarch::predict(media_variancia_condicional, n.ahead = 3)


MODELO_SIMPLES = garchFit (~ arma (1, 0) + garch (1, 1), data = log_day_return, 
                           cond.dist = "sstd", trace = F, include.skew = F,
                           include.shape = F)
summary (MODELO_SIMPLES)

MODELO_SIMPLES = garchFit (~ arma (1, 1) + garch (2, 1), data = log_day_return, 
                           cond.dist = "sstd", trace = F, include.skew = F,
                           include.shape = F)


#EXERCÍCIO 01: Escolha uma ação brasileira e faça a estimação do 
...
Série_01 = garchSim(spec, n = 10000)

write.table(Série_01, file = "/cloud/project/3.Series_Temporais/3.Serie_Temporal_Univariada/7.GARCH/Série_01.csv", sep = ",", na = "", quote = TRUE, row.names = FALSE)

DADOS <- read_csv("/cloud/project/3.Series_Temporais/3.Serie_Temporal_Univariada/7.GARCH/Série_01.csv")
